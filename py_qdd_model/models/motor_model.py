...
class MotorModel:
    ...
    def _iterate_thermal_equilibrium(self, state: Dict[str, npt.NDArray], iters: int, relax: float) -> Dict[str, npt.NDArray]:
        ...
        for _ in range(iters):
            ...
            Bmax = self._estimate_flux_density(state["motor_rpm"], voltage_available=self.p.bus_voltage, current=state["current"], motor_temp=state["motor_temp"])
            ...
            state["phase_resistance"] = self.p.phase_resistance * (1 + C.PhysicsConstants.COPPER_TEMP_COEFF * (state["motor_temp"] - C.PhysicsConstants.REFERENCE_TEMPERATURE))

            if np.all(np.abs(state["motor_temp"] - prev_temp) < C.ModelDefaults.CONVERGENCE_THRESHOLD):
                break
        
        state["Bmax"] = self._estimate_flux_density(state["motor_rpm"], voltage_available=self.p.bus_voltage, current=state["current"], motor_temp=state["motor_temp"])
        return state
